...
public class Helper {
	...
	// ======================retrieve the list of connected pi devices
	// ===================================

	public static ArrayList<NetworkDevice> executeGetPiCamDevices(String command, String currentIp) {
		...
		try {
			...
			String line = "";
			while ((line = reader.readLine()) != null) {
				if (line.indexOf("report") != -1 && line.indexOf("meye") != -1 && line.indexOf(currentIp) == -1) {
					String[] report = line.split(" ");
					if (report.length == 6) {
						NetworkDevice newDevice = new NetworkDevice(report[5].substring(1, report[5].length() - 1),
								report[4]);
	...
	// ======================retrieve the list of connected other devices
	// ===================================

	public static ArrayList<NetworkDevice> executeGetOtherDevices(String command, String currentIp) {
	...
	// =========================Save the discovered devices to
	// Database===============
	...
	// ============================pushing the configuration into the
	// device==================
	public static void executePushConfFile(String host) {
		String command = "sshpass -p 'pass' scp -r /home/pi/conf_files/thread-1.conf admin@" + host + ":/data/etc";
